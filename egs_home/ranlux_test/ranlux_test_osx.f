      ...
      ISEED = 1
      Luxury_level = 1
      WRITE(6,1010)ISEED,Luxury_level
1010  FORMAT(//'                ranlux_test'// ' ISEED =',I16,'  Luxury_
     *level=',I2/)
      call init_ranlux(Luxury_level,ISEED)
      call ranlux(rng_array)
      rng_seed = 1
      WRITE(6,1020)ISEED
1020  FORMAT(' After rng-initialization ISEED=', I17)
      WRITE(6,1030)
...
1060  FORMAT(/' net cputime after loop ', F12.5//)
      SUM = 0.0
      WRITE(6,1070)
1070  FORMAT(' For luxury level 1')
      DO 1081 ILOOP=1,NLOOP
        SUM = SUM + RNG(ILOOP)
        IF ((ILOOP .EQ. 10)) THEN
          WRITE(6,1090)SUM
1090      FORMAT(' Sum of first 10 is  ',F23.8,T53,' should be 6.0412120
     *2')
        END IF
        IF ((ILOOP .EQ. 10000)) THEN
          WRITE(6,1100)SUM
1100      FORMAT(' Sum of first 10,000 is ',F20.8,T50, ' should be 5037.
     *36653233 ')
        END IF
        IF ((ILOOP .EQ. 100000)) THEN
          WRITE(6,1110)SUM
1110      FORMAT(' Sum of first 100,000 is ',F19.8,T49, ' should be 5016
     *7.93485560 ')
        END IF
        IF ((ILOOP .EQ. 1000000)) THEN
          WRITE(6,1120)SUM
1120      FORMAT(' Sum of first 1,000,000 is',F18.8,T48,' should be 5001
     *81.82346285 ')
        END IF
1081  CONTINUE
1082  CONTINUE
      ...
      Luxury_level = 4
      WRITE(6,1130)
1130  FORMAT(///' For luxury level 4')
      call init_ranlux(Luxury_level,ISEED)
      call ranlux(rng_array)
      rng_seed = 1
      cput1=egs_etime()
      WRITE(6,1140)cput1
1140  FORMAT(' cputime at start of loop, ', F12.5)
      DO 1151 ILOOP=1,NLOOP
        IF (( rng_seed .GT. 24 )) THEN
          call ranlux(rng_array)
          rng_seed = 1
        END IF
...
1152  CONTINUE
      ...
      tnow = egs_tot_time(0)
      WRITE(6,1160)tnow
1160  FORMAT(/' Ellapsed wall clock time:',F12.3,' seconds'/)
      WRITE(6,1170)cput2-cput1
1170  FORMAT(/' net cputime after loop ', F12.5//)
      DO 1181 ILOOP=1,NLOOP
        SUM = SUM + RNG(ILOOP)
        IF ((ILOOP .EQ. 10)) THEN
          WRITE(6,1190)SUM
1190      FORMAT(' Sum of first 10 is  ',F23.8,T53,' should be 6.0412120
     *2')
        END IF
        IF ((ILOOP .EQ. 10000)) THEN
          WRITE(6,1200)SUM
1200      FORMAT(' Sum of first 10,000 is ',F20.8,T50, ' should be 5035.
     *82869888')
        END IF
        IF ((ILOOP .EQ. 100000)) THEN
          WRITE(6,1210)SUM
1210      FORMAT(' Sum of first 100,000 is ',F19.8,T49, ' should be 5012
     *2.38526364 ')
        END IF
        IF ((ILOOP .EQ. 1000000)) THEN
          WRITE(6,1220)SUM
1220      FORMAT(' Sum of first 1,000,000 is',F18.8,T48,' should be 5000
     *49.78976676 ')
        END IF
1181  CONTINUE
1182  CONTINUE
      WRITE(6,1230)
1230  FORMAT(///' End ranlux_test')
      call exit(0)
      END
...
1262    CONTINUE
      ...
      WRITE(6,1270)luxury_level,jseed
1270  FORMAT(//' ***************** RANLUX initialization ***************
...
1312  CONTINUE
     ...
     * )) THEN
        WRITE(6,1320)state(25),nskip,i24,j24
1320    FORMAT('// *********** Error in set_ranlux_state: seeds outsideo
...
C*****************************************************************************
     ...
     *4, host_name*64
      INTEGER n_parallel, i_parallel, first_parallel,n_max_parallel, n_c
     *hunk, file_units, n_files,i_input,i_log,i_incoh, i_nist_data,i_msc
     *at,i_photo_cs,i_photo_relax, xsec_out
     ...
     *s_opened
      ...
      DO 1331 i=1,len(line)
        line(i:i) = '='
1331  CONTINUE
1332  CONTINUE
      DO 1341 i=1,len(line1)
        line1(i:i) = '.'
1341  CONTINUE
1342  CONTINUE
     ...
     *gs file'
            ...
            call exit(1)
          END IF
          open(kmpi,file=pegs_file,status='old',err=1350)
          goto 1360
        END IF
...
1382  CONTINUE
     ...
     * file ', tmp1_string(:lnblnk1(tmp1_string))
          ...
          call exit(1)
        END IF
        open(i_photo_cs,file=tmp1_string,status='old',err=1390)
      ELSE
        i_photo_cs = itmp
      END IF
      DO 1401 i=1,len(tmp1_string)
        tmp1_string(i:i) = ' '
1401  CONTINUE
1402  CONTINUE
     ...
     * file ', tmp1_string(:lnblnk1(tmp1_string))
          ...
          call exit(1)
        END IF
        open(i_mscat,file=tmp1_string,status='old',err=1390)
      ELSE
        i_mscat = itmp
      END IF
      DO 1411 i=1,len(tmp1_string)
        tmp1_string(i:i) = ' '
1411  CONTINUE
1412  CONTINUE
     ...
     * file ', tmp1_string(:lnblnk1(tmp1_string))
          ...
          call exit(1)
        END IF
        open(i_incoh,file=tmp1_string,status='old',err=1390)
      ELSE
        i_incoh = itmp
      END IF
      DO 1421 i=1,len(tmp1_string)
        tmp1_string(i:i) = ' '
1421  CONTINUE
1422  CONTINUE
     ...
     * file ', tmp1_string(:lnblnk1(tmp1_string))
          ...
          call exit(1)
        END IF
        open(i_photo_relax,file=tmp1_string,status='old',err=1390)
      ELSE
        i_photo_relax = itmp
      END IF
      DO 1431 i=1,len(ucode_dir)
        ucode_dir(i:i) = ' '
1431  CONTINUE
1432  CONTINUE
     ...
     *    ' does not exist.'
          ...
          call exit(1)
        END IF
        open(i_input,file=tmp_string,status='old',err=1440)
      END IF
      DO 1451 i=1,len(work_dir)
        work_dir(i:i) = ' '
1451  CONTINUE
1452  CONTINUE
      work_dir = 'egsrun_'
...
1532  CONTINUE
     ...
     * file'
          ...
          call exit(1)
        END IF
        open(i_help,file=tmp_string,status='old',err=1540)
1551    CONTINUE
          read(i_help,'(a)',err=1560,end=1560) line1
          write(i_log,'(a)') line1
        GO TO 1551
1552    CONTINUE
1560    CONTINUE
...
1582  CONTINUE
      IF (( have_arg )) THEN
        read(arg,*,err=1590) n_parallel
        IF((n_parallel .LT. 0))goto 1590
        goto 1600
1590    CONTINUE
        write(i_log,'(/a)') '***************** Warning: '
...
1612  CONTINUE
      IF (( have_arg )) THEN
        read(arg,*,err=1620) i_parallel
        IF((i_parallel .LT. 0))goto 1620
        goto 1630
1620    CONTINUE
        write(i_log,'(/a)') '***************** Warning: '
...
1642  CONTINUE
      IF (( have_arg )) THEN
        read(arg,*,err=1650) first_parallel
        IF((first_parallel .LT. 1))goto 1650
        goto 1660
1650    CONTINUE
        write(i_log,'(/a)') '***************** Warning: '
...
1722  CONTINUE
     ...
     *de_dir*1024, input_line*100, arg*20
      ...
      logical ex,is_open
      DO 1731 i=1,len(tmp_string)
        tmp_string(i:i) = ' '
1731  CONTINUE
1732  CONTINUE
      DO 1741 i=1,len(ucode_dir)
        ucode_dir(i:i) = ' '
1741  CONTINUE
1742  CONTINUE
      ucode_dir = egs_home(:lnblnk1(egs_home)) // user_code(:lnblnk1(use
...
1752  CONTINUE
      ...
      IF (( is_batch )) THEN
        ...
        open(i_log,file=tmp1_string,status='unknown',err=1760)
      END IF
      DO 1771 i=1,len(tmp2_string)
        tmp2_string(i:i) = ' '
1771  CONTINUE
1772  CONTINUE
     ...
     *file'
          ...
          call exit(1)
        END IF
        open(i_iofile,file=tmp2_string,status='old',err=1780)
1791    CONTINUE
          read(i_iofile,'(a)',err=1800,end=1800) input_line
          IF((input_line(1:1) .EQ. '#'))GO TO1791
          read(input_line,*,err=1810,end=1810) u
          istart = 1
          DO 1821 i=lnblnk1(input_line),1,-1
...
1831      CONTINUE
1832      CONTINUE
          DO 1841 i=istart,lnblnk1(input_line)
...
1842      CONTINUE
     ...
     *as $mx_units and retry'
            ...
            DO 1851 i=1,len(file_extensions(n_files))
...
1882  CONTINUE
      IF (( lnblnk1(work_dir) .EQ. 0 )) THEN
        return
      END IF
      DO 1891 i=1,len(base)
        base(i:i) = ' '
1891  CONTINUE
1892  CONTINUE
      base = egs_home(:lnblnk1(egs_home)) // user_code(:lnblnk1(user_cod
     *e)) // '/' // work_dir(:lnblnk1(work_dir))
      DO 1901 i=1,lnblnk1(base)
        IF (( base(i:i) .EQ. '/' )) THEN
          base(i:i) = '/'
        END IF
1901  CONTINUE
1902  CONTINUE
      IF (( egs_isdir(base) )) THEN
        DO 1911 i=1,len(tmp_string)
          tmp_string(i:i) = ' '
1911    CONTINUE
1912    CONTINUE
        DO 1921 i=1,len(junk_file)
          junk_file(i:i) = ' '
1921    CONTINUE
1922    CONTINUE
...
1972  CONTINUE
     ...
     *,2)
      CHARACTER*4 ASYM
      REAL DL1,DL2,DL3,DL4,DL5,DL6,   ALPHI,  BPAR,  DELPOS,  WA,  PZ,
     *ZELEM,  RHOZ,  PWR2I,  DELCM,  ZBRANG,  LZBRANG
      INTEGER NNE,  IBRDST,  IPRDST,  ibr_nist,  itriplet,  pair_nrc
     ...
     *_shell(10), radc_flag,  ibcmp(2000)
      INTEGER iz_array,ne_array,shn_array,eno_atbin_array, shell_array,n
     *_shell,radc_flag
     ...
     *L(2000),IPHTER(2000)
      REAL binding_energies,  interaction_prob,    relaxation_prob,  edg
     ...
     *10), sig_ismonotone(0:1,10)
      REAL esig_e,   psig_e,   esige_max,  psige_max,  range_ep,  E_arra
     *y,  etae_ms0,etae_ms1,  etap_ms0,etap_ms1,  q1ce_ms0,q1ce_ms1,  q1
     *cp_ms0,q1cp_ms1,  q2ce_ms0,q2ce_ms1,  q2cp_ms0,q2cp_ms1,  blcce0,b
     *lcce1,   expeke1,  EKE0,EKE1, XR0,  TEFF0,  BLCC,  XCC,  ESIG0,ESI
     *G1,  PSIG0,PSIG1,  EDEDX0,EDEDX1,  PDEDX0,PDEDX1,  EBR10,EBR11,  P
     *BR10,PBR11,  PBR20,PBR21,  TMXS0,TMXS1
      INTEGER IUNRST,  EPSTFL,  IAPRIM
     ...
     *,v_final,w_final, IDISC,IROLD,IRNEW,IAUSFL(35)
      ...
      REAL TSTEP,  TUSTEP,  USTEP,  VSTEP,  TVSTEP,  RHOF,  EOLD,  ENEW,
     *  EKE,  ELKE,  GLE,  E_RANGE, x_final,y_final,z_final,  u_final,v_
     *final,w_final
     ...
     *EM(1,10), NGR(10)
      REAL EBINDA,  GE0,GE1,  GMFP0,GMFP1,  GBR10,GBR11,  GBR20,GBR21,
     *RCO0,RCO1,  RSCT0,RSCT1,  COHE0,COHE1,   PHOTONUC0,PHOTONUC1,  DPM
     *FP
      INTEGER
     *                MPGEM,
     *                        NGR
     ...
     *(10)
      REAL RMT2,  RMSQ,  AP,  AE,  UP,  UE,  TE,  THMOLL
     ...
     *ii_no(10,50),  eii_flag
      REAL eii_xsection_a,eii_xsection_b,eii_a,eii_b,eii_cons,eii_L_fact
     *or
     ...
     *, By, Bz,  Bx_new, By_new, Bz_new,  emfield_on
      REAL ExIN,EyIN,EzIN, EMLMTIN, BxIN,ByIN,BzIN, Bx,By,Bz, Bx_new,By_
     *new,Bz_new
      logical emfield_on
...
1982  CONTINUE
     ...
     *THEN
      ...
      DO 1991 i=1,10
        iraylm(i) = 0
        DO 2001 j=1,len(iray_ff_file(i))
          iray_ff_file(i)(j:j) = ' '
2001    CONTINUE
2002    CONTINUE
        DO 2011 j=1,len(iray_ff_media(i))
          iray_ff_media(i)(j:j) = ' '
2011    CONTINUE
2012    CONTINUE
        ae(i)=0
...
2092    CONTINUE
      ...
      DO 2101 i=1,len(input_file)
        input_file(i:i) = ' '
2101  CONTINUE
2102  CONTINUE
      DO 2111 i=1,len(output_file)
        output_file(i:i) = ' '
2111  CONTINUE
2112  CONTINUE
      DO 2121 i=1,len(work_dir)
        work_dir(i:i) = ' '
2121  CONTINUE
2122  CONTINUE
      DO 2131 i=1,len(pegs_file)
        pegs_file(i:i) = ' '
2131  CONTINUE
2132  CONTINUE
      DO 2141 i=1,len(host_name)
        host_name(i:i) = ' '
2141  CONTINUE
2142  CONTINUE
      n_parallel = 0
     ...
     * text_string
      ...
      logical ex,iwin
      iwin=.false.
      DO 2151 i=1,len(base)
        base(i:i) = ' '
2151  CONTINUE
2152  CONTINUE
      base = egs_home(:lnblnk1(egs_home)) // user_code(:lnblnk1(user_cod
     *e)) // '/' // output_file(:lnblnk1(output_file)) // '_w'
      DO 2161 i=1,len(base1)
        base1(i:i) = ' '
2161  CONTINUE
2162  CONTINUE
...
2182  CONTINUE
     ...
     *arallel runs.'
      ...
      IF ((iwin)) THEN
        read(ipar,'(a)',err=2190,end=2190) text_string
        text_string = text_string(:lnblnk1(text_string))
        textindex = index(text_string,'File(s)')
        text_string = text_string(:textindex-1)
        read(text_string,'(i256)',err=2190) numparfiles
      ELSE
        read(ipar,'(i256)',err=2190,end=2190) numparfiles
      END IF
      close(ipar)
      DO 2201 i=1,len(command)
        command(i:i) = ' '
2201  CONTINUE
2202  CONTINUE
      IF ((iwin)) THEN
     ...
     *llel runs.'
      ...
      k=1
      j=1
2211  IF(j.GT.numparfiles)GO TO 2212
        DO 2221 i=1,len(tmp_string)
          tmp_string(i:i) = ' '
2221    CONTINUE
2222    CONTINUE
...
2262  CONTINUE
     ...
     *opened', ' by specifying it in the .io file.'
      ELSE IF(( rl .EQ. 0 )) THEN
        open(the_unit,file=tmp_string,status='unknown',err=2270)
      ELSE
        open(the_unit,file=tmp_string,status='unknown',form='unformatted
     *', access='direct', recl=rl,err=2270)
      END IF
      egs_open_file = the_unit
      return
2270  error_string = 'In egs_open_file: failed to open file ' // tmp_str
     ...
     *at,i_photo_cs,i_photo_relax, xsec_out
      ...
      IF (( egs_is_absolute_path(extension) )) THEN
        IF (( rl .EQ. 0 )) THEN
          open(the_unit,file=extension,status='old',err=2280)
        ELSE
          open(the_unit,file=extension,status='old',form='unformatted',
     *    access='direct',recl=rl,err=2280)
        END IF
...
2302  CONTINUE
     ...
     *  // extension(:lnblnk1(extension))
      END IF
      IF (( rl .EQ. 0 )) THEN
        open(the_unit,file=fn,status='old',err=2310)
      ELSE
        open(the_unit,file=fn,status='old',form='unformatted',access='di
     *rect', recl=rl,err=2310)
      END IF
      egs_open_datfile = the_unit
      return
2310  CONTINUE
...
2322  CONTINUE
     ...
     *   extension(:lnblnk1(extension))
      END IF
      IF (( rl .EQ. 0 )) THEN
        open(the_unit,file=fn,status='old',err=2330)
      ELSE
        open(the_unit,file=fn,status='old',form='unformatted',access='di
     *rect', recl=rl,err=2330)
      END IF
      egs_open_datfile = the_unit
      return
2330  CONTINUE
...
2342    CONTINUE
        ...
        END IF
      END IF
      open(the_unit,file=filen,status='old',err=2350)
      egs_open_file_junk = the_unit
      return
2350  egs_open_file_junk = -3
      return
      end
...
2462  CONTINUE
      ...
      l = 24
      DO 2471 l=24,1,-1
        IF((media(l,imed)(1:1) .NE. ' '))GO TO2472
2471  CONTINUE
2472  CONTINUE
      l = min(l,len(medname))
      DO 2481 i=1,l
...
2542  CONTINUE
      ...
      ELSE
        ml = m_lower
        mu = m_upper
2551    IF(iabs(mu-ml).LE.1)GO TO 2552
          mav = (ml+mu)/2
          IF (( s .LT. x(mav) )) THEN
            mu = mav
          ELSE
      ...
      DO 2571 i=1,nsbin-1
        DO 2581 j_h=1,nsbin
          IF (( ws_array(j_h) .LT. 0 )) THEN
            IF((abs(ws_array(j_h)) .GT. sum))GOTO 2590
          END IF
2581    CONTINUE
2582    CONTINUE
        j_h = nsbin
2590    CONTINUE
          ...
          IF (( ws_array(j_l) .LT. 0 )) THEN
            IF((abs(ws_array(j_l)) .LT. sum))GOTO 2600
          END IF
2591    CONTINUE
2592    CONTINUE
        j_l = nsbin
2600    aux = sum - abs(ws_array(j_l))
...
2572  CONTINUE
      ...
      REAL function alias_sample1(nsbin,xs_array,fs_array,ws_array,ibin_
     *array)
      implicit none
      integer nsbin
      INTEGER ibin_array(nsbin)
...
2732  CONTINUE
      ...
      toUpper = a_string
      the_string = a_string
      DO 2741 i=1,lnblnk1(the_string)
...
2772  CONTINUE
      ...
      max = nsh
      x = a
2781  IF(min.GE.max-1)GO TO 2782
        help = (max+min)/2
        IF (( b(help).le.x)) THEN
          min = help
        ELSE
